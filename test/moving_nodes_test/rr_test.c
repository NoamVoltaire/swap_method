...
int	main(void)
{
	...
	fillstack(&b, ft_split("6 7 8 9 10", ' '), 5);
		...
		ss(&a, &b);
	fillstack(&good_a, ft_split("2 1 3 4 5", ' '), 5);
	fillstack(&good_b, ft_split("7 6 8 9 10", ' '), 5);
		printf("%d\n", iterate_and_check(a, b, good_a, good_b, 5, 5));
	free_stack(&good_a);
	free_stack(&good_b);

		rr(&a, &b);
	fillstack(&good_a, ft_split("1 3 4 5 2", ' '), 5);
	fillstack(&good_b, ft_split("6 8 9 10 7", ' '), 5);
		printf("%d\n", iterate_and_check(a, b, good_a, good_b, 5, 5));
	free_stack(&good_a);
	free_stack(&good_b);
	...
	fillstack(&good_b, ft_split("10 7 6 8 9", ' '), 5);
		printf("%d\n", iterate_and_check(a, b, good_a, good_b, 5, 5));
	free_stack(&good_a);
	free_stack(&good_b);
