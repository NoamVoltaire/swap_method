...
/*   Updated: 2024/03/17 02:37:42 by noam             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	rotate_single(t_stack **lst, char stack_name, int *rotation)
{
	while (*rotation > 0 && *rotation--)
		rotate(lst, stack_name);
	while (*rotation < 0 && *rotation++)
		reverse_rotate(lst, stack_name);
}
...
void	rotate_before_insert(t_stack **a, t_stack **b, t_global global)
{
	...
	rotate_single(a, 'a', &(global.target_node->a_rot));
	rotate_single(b, 'b', &(global.target_node->b_rot));
}
